...
class Webcam:
    ...
    def __init__(self, add, h, w):
        ...
        try:
            ...
            #The start box (x, y), which we will draw latter
        ...
        try:
            ...
            #The end box (x, y), which we will draw latter
    ...
    def killcam(self):
        ...
        """
        try:
            cv2.destroyWindow(self.cam_name) #In some version of openCV, this give an error
        except:
            self.key = 27 #Equivalent to 'ESC'
        self.active = False
        self.cap.release()
    
    def is_quit(self):
        """
        function to check if user try to close the webcam window
        """
        if self.key==27: # key =27 is the ESC key
            self.killcam() # kill this Webcam
        elif cv2.getWindowProperty(self.cam_name, cv2.WND_PROP_VISIBLE) < 1: # cv2.WND_PROP_VISIBLE = 0 when user click on "X" on the camera window
            self.killcam() # kill this Webcam
    
    def mouse_and_keys(self, event, x, y, flags, param):
        """
        function to get mouse event in the webcam window
        """
        ...
        if event == cv2.EVENT_MOUSEMOVE:
            ...
            if self.draw_count%2 == 0: #True if we drawing the 'start point'
                self.start_y1 = self.y1_draw 
                self.start_x1 = self.x1_draw
            else: #Ortherwise, we drawing the 'end point'
            ...
            if self.draw_count%2 == 0: #True if we drawing the 'start point'
                ...
                #Then save these point as "start_box.txt"
                ...
                #Then save these point as "end_box.txt"
    ...
    def display(self, img, delay = 10, see_clock =True):
        ...
        Args:
        img: an image
        waittime (ms): delay time waiting for any keys press, also work as a delay between images to be displayed
        ...
        # Need to draw the estimate bounding box for startting and ending point 
        if self.drawing:
        ...
        self.sbb = [self.start_x1,self.start_y1,self.start_x2,self.start_y2]
        self.ebb = [self.end_x1,self.end_y1,self.end_x2,self.end_y2]
